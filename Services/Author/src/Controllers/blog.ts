...
export const createBlog = TryCatch(async (req: authenticateRequest, res) => {
  /* ------------------------------------------------------------------ *
   * 1.  Validate body ------------------------------------------------- */
...
export const updateBlog = TryCatch(async (req: authenticateRequest, res) => {
  const { id } = req.params; 
...
export const deleteBlog = TryCatch(async(req:authenticateRequest,res)=>{
  const {id} = req.params;
  const userId = req.user;
