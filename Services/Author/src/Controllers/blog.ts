...
import { authenticateRequest } from "../Middlewares/isAuth.js";
...
export const updateBlog = TryCatch(async (req: authenticateRequest, res):Promise<void> => {
  const { id } = req.params; 
...
export const deleteBlog = TryCatch(async(req:authenticateRequest,res):Promise<void>=>{
  const {id} = req.params;
  const userId = req.user;
...
export const aiTitleResponse = TryCatch(async (req, res):Promise<void> => {
  const { text } = req.body;
...
export const aiDescriptionResponse = TryCatch(async (req, res):Promise<void> => {
...
export const aiBlogResponse = TryCatch(async (req, res):Promise<void> => {
  const { blog } = req.body as { blog?: string };
